    ...
    >
      ...
      <template slot="table-row" slot-scope="props">
        ...
        <span v-else-if="props.column.field == 'title'">
          <a v-bind:href="props.row.uri" target="_new">{{
            props.row.title.replace(/_/g, " ")
          }}</a>
        </span>
  ...
  methods: {
    ...
    onCellClick(params) {
      if (params.column.field == "id") {
        console.log(params.row._id, params.row.title, this.rating);
        let url = this.props.remoteURL + "/" + params.row._id;
    ...
    loadItems() {
      ...
      axios.get(url, { body: this.serverParams }).then(response => {
